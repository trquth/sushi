...
//New
shoppingCart.prototype.newLoadItems = function () {
    var items = localStorage != null ? localStorage[this.CartName + "_items"] : null;
    if (items != null && JSON != null) {
        try {
            var items = JSON.parse(items);
            for (var i = 0; i < items.length; i++) {
                ...
                if (item.product != null && item.quantity != null) {
                    item = new newCartItem(item.product, item.quantity);
                    this.items.push(item);
                }
            }
        }
...
//New
shoppingCart.prototype.newSaveItems = function () {
    if (localStorage != null && JSON != null) {
        localStorage[this.newCartName + "_items"] = JSON.stringify(this.items);
    }
}
...
//New
shoppingCart.prototype.newAddItem = function (product, quantity) {
    quantity = this.toNumber(quantity);
    if (quantity != 0) {
        ...
        // new item, add now
        if (!found) {
            var item = new newCartItem(product, quantity);
            this.items.push(item);
        }
...
//New
function newCartItem(product,quantity) {
